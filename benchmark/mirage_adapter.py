...
MIR2TAR_PATH = os.path.join(MIRNA_DATA_DIR, "mir2tar.csv")
BIOGENESIS_PATH = os.path.join(MIRNA_DATA_DIR, "biogenesis.csv")
RISC_PATH = os.path.join(MIRNA_DATA_DIR, "risc.csv")
SORTING_PATH = os.path.join(MIRNA_DATA_DIR, "sorting.csv")
GENEINFO_PATH = os.path.join(MIRNA_DATA_DIR, "geneinfo.csv")
...
def run_mirage_pipeline(adata: ad.AnnData, out_dir: str, config: Dict,
    ...
    if ground_truth is not None:
        ...
        if n_true_s < cfg.TOP_S_SENDERS and n_true_r < cfg.TOP_R_RECEIVERS:
            print(f"    Active cells should be covered by graph")
        else:
            print(f"    Active cells may exceed graph coverage!")
